...
static struct i2c_board_info __initdata kd_lens_dev={ I2C_BOARD_INFO("OV8835AF", 0x6c)};


#define OV8835AF_DRVNAME "OV8835AF"
#define OV8835AF_VCM_WRITE_ID           0x6c
...
#define OV8835AF_write_cmos_sensor(addr, para) iWriteReg((u16) addr , (u32) para , 1, OV8835AF_VCM_WRITE_ID)

kal_uint16 OV8835AF_read_cmos_sensor(kal_uint32 addr)
{
kal_uint16 get_byte=0;
    iReadReg((u16) addr ,(u8*)&get_byte,OV8835AF_VCM_WRITE_ID);
    return get_byte;
}
...
static int s4OV8835AF_ReadReg(unsigned short * a_pu2Result)
{
    ...
    temp = (OV8835AF_read_cmos_sensor(0x3618)+ (OV8835AF_read_cmos_sensor(0x3619)<<8))>>4;

    *a_pu2Result = temp;
	OV8835AFDB("s4OV8835AF_ReadReg = %d \n", temp);
    return 0;
}
...
{
    u16 temp,SlewRate=0;
    OV8835AFDB("s4OV8835AF_WriteReg = %d \n", a_u2Data);
	
	temp=(a_u2Data<<4)+8+SlewRate;
	OV8835AFDB("-----stemp=(a_u2Data<<4)+8+SlewRate = %d----- \n", temp);
	
	OV8835AF_write_cmos_sensor(0x3619,(temp>>8)&0xff);
	
	OV8835AF_write_cmos_sensor(0x3618,temp&0xff);
